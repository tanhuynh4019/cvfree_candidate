<template>
    <div>
        <v-row class="mt-6">
            <v-col cols="12" class="text-center" v-if="jobs.length == 0">
                Hiện tại {{ website.company }} không tìm được vị trí công việc này! Bạn có thể tìm một
                công việc khác
            </v-col>
            <v-col cols="12" v-for="job in jobs" :key="job._id">
                <div class="blog-card" :style="`border: 1px dashed ${website.color.redMain.color};`">
                    ...
                    <div class="description">
                        <router-link style="text-decoration: none;"
                            :to="{ path: `/tim-viec-lam/review-viec-lam/${job.slug}` }" @click.native="scrollToTop()">
                            <h1>
                                {{ job.name }}
